...
.tile {cursor:pointer;background:#fff;border:1px solid var(--line);border-radius:11px;padding:13px 15px;
       box-shadow:0 2px 8px rgba(255,153,0,.08);margin-bottom:10px;}
...
def load_file(path_str):
    ...
    out = {"file": path.name, "path": str(path), "date": parse_file_date(path.name)}
    if "Roster" in sheets:
...
def load_all():
    files = []
    # Check both current repo directory and AUH 1 subfolder
    search_dirs = [APP_DIR, APP_DIR / "AUH 1", APP_DIR / "AUH1"]
    seen_names = set()
    for s_dir in search_dirs:
        if s_dir.exists():
            for p in s_dir.glob("DWD-AUH1-*.xlsx"):
...
bundle = load_all()
if not bundle:
    st.error("No valid DWD-AUH1-DDMMYYYY.xlsx files were found in the root repository or 'AUH 1' folder.")
    st.stop()
...
c1, c2, c3, c4 = st.columns([1.05, 1.25, 2.05, 0.9])
with c1:
...
if nav in ["Overview","DWD Intelligence","SL / PL Analysis","Attendance Trends","Compliance Matrix","Reports"]:
    left, right = st.columns([1.02, 0.98], gap="small")
    ...
    with left:
        ...
        with m2:
            st.markdown(f'<div class="metric"><div class="label">Active Rate</div><div class="value">{fmt_pct(active_rate)}</div><div class="delta">● Calculated from files</div></div>', unsafe_allow_html=True)
        with m3:
            st.markdown(f'<div class="metric"><div class="label">Compliance Rate</div><div class="value">{fmt_pct(compliance)}</div><div class="delta down">● Present / scheduled roster</div></div>', unsafe_allow_html=True)
        ...
        if st.session_state.get("dwd_open", False) or nav == "DWD Intelligence":
            ...
            with a:
                ...
                for dt, g in h.groupby("_file_date"):
                    ...
                    day_rows.append({
                        ...
                        "Associates": len(g),
                        "Present": int((aa == "P").sum()),
                        "SL": int((aa == "SL").sum()),
                        "PL": int((aa == "PL").sum()),
                        "Absent": int((aa == "AB").sum()),
                        "OFF": int((aa == "OFF").sum())
                    })
                if day_rows:
            ...
            with b:
                ...
                if ag:
                    ...
                    fig.update_layout(height=270, margin=dict(l=10, r=10, t=10, b=45), paper_bgcolor="white", plot_bgcolor="white", font=dict(color="#173252", size=11), showlegend=False)
            ...
            with a:
                ...
                if not h.empty:
                    ...
                    fig.update_layout(height=260, margin=dict(l=10, r=10, t=10, b=35), paper_bgcolor="white", plot_bgcolor="white", font=dict(color="#173252", size=11), legend=dict(orientation="h", y=1.12, x=0))
                    st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
                else:
            ...
            with b:
                ...
                fig = go.Figure(go.Pie(labels=labels, values=values, hole=0.62, textinfo="percent"))
                fig.update_layout(height=260, margin=dict(l=0, r=0, t=10, b=0), paper_bgcolor="white", legend=dict(font=dict(size=10)), showlegend=True)
                st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    ...
    with right:
        ...
        k1, k2 = st.columns(2)
        with k1:
            st.markdown(f'<div class="metric"><div class="label">Single-Day Strategic Leaves</div><div class="value">{sl}</div><div>SL records in loaded source</div><div class="delta up">● Active filter count</div></div>', unsafe_allow_html=True)
        with k2:
            st.markdown(f'<div class="metric"><div class="label">2-Day Consecutive Clusters</div><div class="value">{"—" if len(bundle)<2 else "Calculated"}</div><div>{"Requires ≥2 dated files" if len(bundle)<2 else "See pattern table"}</div><div class="delta">● Multi-day analysis</div></div>', unsafe_allow_html=True)
        ...
        if not h.empty:
            ...
            fig.update_layout(barmode="stack", height=300, margin=dict(l=10, r=10, t=20, b=40), paper_bgcolor="white", plot_bgcolor="white", font=dict(color="#173252", size=11), legend=dict(orientation="h", y=1.08, x=0))
            st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
        else:
        ...
        if idc and nc and len(bundle) >= 2:
            ...
            if not drill.empty:
                ...
                st.info("No repeated SL/PL pattern meets the review threshold in the selected files.")
        else:
            st.info("High-frequency detection requires at least 2 dated DWD files.")

if nav == "Settings":
    st.markdown('<div class="card"><div class="card-title">Data Source Settings</div><p class="sub">The app reads files matching <b>DWD-AUH1-DDMMYYYY.xlsx</b>.</p></div>', unsafe_allow_html=True)
...
if nav == "Reports":
    ...
    if bundle:
        st.write("Workbook sheets detected:", ", ".join(pd.ExcelFile(bundle[0]["path"]).sheet_names))
